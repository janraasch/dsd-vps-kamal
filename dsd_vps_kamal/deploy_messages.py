...
- Run `kamal setup` to deploy your project to your VPS.
- Open your deployed project in a new browser tab.

Note: This will automatically accept the server's SSH host key if it
hasn't been seen before (StrictHostKeyChecking=accept-new), adding it
...
def success_msg(log_output=""):
    ...
    msg = dedent(
        """
        --- Your project is now configured for deployment to your VPS using Kamal. ---
        ...
        - As you develop your project further:
            ...
            - Run `kamal deploy` to push your changes.
    """
    )
...
def success_msg_automate_all(deployed_url):
    ...
    msg = dedent(
        ...
        If you make further changes and want to push them to your VPS,
        commit your changes and then run `kamal deploy`.
    """
    )
    return msg
